...
export default function SettingsPage() {
  ...
  const loadPaymentSettings = async () => {
    ...
    if (data) {
      ...
      setCurrency((data as any).default_currency || "INR");
  ...
  const savePaymentSettings = async () => {
    ...
    const { error } = await supabase
      .from("coach_payment_settings" as any)
      .upsert({ coach_id: user.id, razorpay_key_id: razorpayKeyId.trim(), razorpay_key_secret: razorpayKeySecret.trim(), default_currency: currency, updated_at: new Date().toISOString() } as any, { onConflict: "coach_id" });
  ...
  const saveCurrency = async (value: string) => {
    ...
    const { error } = await supabase
      .from("coach_payment_settings" as any)
      .upsert({ coach_id: user.id, default_currency: value, updated_at: new Date().toISOString() } as any, { onConflict: "coach_id" });
    if (error) toast({ title: "Error", description: error.message, variant: "destructive" });
    else toast({ title: "Currency updated" });
          ...
          <TabsContent value="payments">
            <Card className="card-shadow mb-4">
              ...
              <CardContent>
                ...
                <Select value={currency} onValueChange={saveCurrency} disabled={savingCurrency}>
                ...
                <p className="text-xs text-muted-foreground mt-1.5">Used as the default currency for your services and pricing.</p>
              </CardContent>
            </Card>
            <Card className="card-shadow">
              <CardHeader>
